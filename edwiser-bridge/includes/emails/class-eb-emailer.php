...
class Eb_Emailer {
	...
	public function refund_completion_email( $args ) {
		...
		if ( $eb_general ) {
			$send_email_to_admin        = wdm_eb_get_value_from_array( $eb_general, 'eb_refund_mail_to_admin', false );
			$specified_email_for_refund = wdm_eb_get_value_from_array( $eb_general, 'eb_refund_mail', false );
		}
		...
		if ( false !== $allow_notify && 'ON' === $allow_notify ) {
			if ( $user_email_tmpl_data ) {
				$user               = get_user_by( 'id', wdm_eb_get_value_from_array( $args, 'buyer_id' ), '' );
