...
if (!class_exists("Eb_Email_Tmpl_Parser")) {
	...
	class Eb_Email_Tmpl_Parser
	{
		...
		private function get_tmpl_constant($args)
		{
			...
			$constant["{USER_ACCOUNT_PAGE_LINK}"] = "<a href='" . wdm_user_account_url() . "'>" . __('User Account', 'eb-textdomain') . "</a>";
		...
		// {
		//     return get_arr_value($args, "refunded_cur", "") . get_arr_value($args, "refund_amount", "0.00");
		// }
		...
		private function get_total_refunded_amt($args)
		{
			...
			$order_id         = get_arr_value($args, "eb_order_id", false);
			if ($order_id) {
//                $ordMeta = new Eb_Order_Meta($this->plugin_name, $this->version);
