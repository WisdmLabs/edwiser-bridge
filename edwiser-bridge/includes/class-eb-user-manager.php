...
class EBUserManager
{
        ...
        if ($connected['success'] == 1) {
            ...
            foreach ($all_users as $key => $value) {
                ...
                if (isset($sync_options['eb_synchronize_user_courses']) &&
                    ...
                    $moodle_user_courses = edwiserBridgeInstance()->course_manager()->get_moodle_courses($value['moodle_user_id']);
                    ...
                    if ($moodle_user_courses['success'] == 1) {
                        foreach ($moodle_user_courses['response_data'] as $course_data) {
                            ...
                            $existing_course_id = edwiserBridgeInstance()->course_manager()->is_course_presynced($course_data->id);
    ...
    public function isMoodleUsernameAvailable($username)
    {
        ...
        $response = edwiserBridgeInstance()->connection_helper()->connectMoodleWithArgsHelper($webservice_function, $request_data);
