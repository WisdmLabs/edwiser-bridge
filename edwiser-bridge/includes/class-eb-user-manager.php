...
class EBUserManager {
	...
	public function create_wordpress_user( $email, $firstname, $lastname, $role = '' ) {
		...
		if ( empty( $email ) || ! is_email( $email ) ) {
			return new \WP_Error( 'registration-error', esc_html__( 'Please provide a valid email address.', 'eb-textdomain' ) );
		}
	...
	public function link_user_bulk_actions_handler() {
		...
		$users = isset( $_REQUEST['users'] ) ? wdm_eb_edwiser_sanitize_array( $_REQUEST['users'] ) : array(); // WPCS: input var ok, CSRF ok, sanitization ok.
