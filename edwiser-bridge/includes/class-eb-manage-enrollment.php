...
if (!class_exists('\app\wisdmlabs\edwiserBridge\EBManageUserEnrollment')) {
    ...
    class EBManageUserEnrollment extends \WP_List_Table
    {
        ...
        public function outPut()
        {
            ...
            <div class="eb-manage-user-enrol-wrap">
                ...
                <div class="eb-notices"><!-- Add custom notices inside this. --></div>
        ...
        public function bpGetTable()
        {
            ...
            return apply_filters("eb_mange_student_enrollment_table_data",$tblRecords);
        }
         ...
         * @return type
         */
        private function getUserProfileURL($userId = '', $with_a = true, $default = '&ndash;')
        {
            $url = $default;
            $user_info = get_userdata($userId);
            if ($user_info) {
                $edit_link = get_edit_user_link($userId);
                $url = $with_a ? '<a href="' . esc_url($edit_link) . '">' . $user_info->user_login . '</a>' : $edit_link;
            }
        ...
        {
            if ($item['cohort']) {
                $stmt = "<div><p>" . $item['cohort'] . "</p>";
                $stmt.="<lable class='ebbp-cohort-details-link' id=''>" . __("Details", EBBP_TD) . "</lable></div>";
                return $stmt;
            } else {
                return "- - -";
            }
        ...
        protected function column_manager($item)
        {
            if ($item['manager']) {
                return $item['manager'];
            } else {
        ...
        protected function column_manage($item)
        {
            /* var_dump(print_r( $item));
              die(); */
            $uid = get_user_meta($item['user_id'], "moodle_user_id", true);
            return apply_filters('edwiser_unenroll_column_in_manage_enrollment', '<a class="mucp-unenrol" data-user-id="' . $uid . '" data-record-id="' . $item['ID'] . '">' . __('Unenroll', 'eb-textdomain') . '</a>');
        }
        ...
        private function unerollUser($recordIds)
        {
            $adminAjaxInit = new BPAdminAjaxInitiater();
            $count = 0;
            foreach ($recordIds as $id) {
                if (isset($id)) {
                    if ($adminAjaxInit->processUnenrollment($id) === 1) {
                        $count++;
                    }
                }
            }
            echo '<div class="notice notice-success is-dismissible">';
            echo '<p>' . sprintf(__("%s records updated out of the %s", 'eb-textdomain'), $count, count($recordIds)) . '</p>';
            echo '</div>';
        }
    }
}
