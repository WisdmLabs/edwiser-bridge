...
class Eb_Order_Status {
	...
	public function init_eb_order_refund() {
		...
		$refund_status = wdm_eb_get_value_from_array( $refund, 'status', false );
		$refund_msg    = wdm_eb_get_value_from_array( $refund, 'msg', '' );
		if ( $refund_status ) {
	...
	public function save_new_order_place_note( $order_id ) {
		...
		$course_id  = wdm_eb_get_value_from_array( $ord_detail, 'course_id' );
	...
	private function get_status_update_note( $order_id, $data ) {
		$ord_detail = get_post_meta( $order_id, 'eb_order_options', true );
		$order_data = wdm_eb_get_value_from_array( $data, 'eb_order_options', false );
		if ( false === $order_data ) {
			return;
		}
		$old_status = wdm_eb_get_value_from_array( $ord_detail, 'order_status', false );
		$new_status = wdm_eb_get_value_from_array( $order_data, 'order_status', false );
		$msg        = array(
	...
	private function get_order_refund_status_msg( $order_id, $data ) {
		$refund_amt = wdm_eb_get_value_from_array( $data, 'amt' );
		$msg        = array(
			'amt'                   => $refund_amt,
			'refund_note'           => wdm_eb_get_value_from_array( $data, 'note' ),
			'refund_unenroll_users' => wdm_eb_get_value_from_array( $data, 'unenroll_users', false ),
		);
		if ( 'ON' === wdm_eb_get_value_from_array( $msg, 'refund_unenroll_users' ) ) {
	...
	private function save_order_refund_amt( $order_id, $refund_amt ) {
		...
		$refund   = array(
			...
			'currency' => wdm_eb_get_current_paypal_currency_symb(),
		);
		if ( is_array( $refunds ) ) {
			$refunds[] = $refund;
		} else {
	...
	private function save_order_status_history( $order_id, $note ) {
		$cur_user = wp_get_current_user();
		wdm_eb_update_order_hist_meta( $order_id, $cur_user->user_login, $note );
	}
	...
	private function unenroll_user_from_courses( $order_id ) {
		...
		$course_id       = wdm_eb_get_value_from_array( $order_details, 'course_id', '' );
		$user_wp_id      = wdm_eb_get_value_from_array( $order_details, 'buyer_id', '' );
