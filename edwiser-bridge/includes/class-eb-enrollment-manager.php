...
class Eb_Enrollment_Manager
{
	...
	{
		return $this->update_user_course_enrollment($args, $roleId);
	}
	...
	public function update_user_course_enrollment($args, $roleId = "5")
	{
		...
		foreach ($moodle_courses as $wpCourseId => $moodle_course_id) {
			// first we check if a moodle course id exists
			if ($moodle_course_id != '') {
				$expireDate = $this->calc_course_acess_expiry_date($wpCourseId);
	...
	**/
	 ...
	 * @return bool true
	 */
	public function update_enrollment_record_wordpress($args, $roleId = "5")
	{
		global $wpdb;
	...
	public function calc_course_acess_expiry_date($course_id)
	{
		...
		if (isset($course_meta["course_expirey"]) && $course_meta["course_expirey"] == "yes") {
...
/**
	...
	public static function access_remianing($userId, $courseId)
	{
		global $wpdb;
