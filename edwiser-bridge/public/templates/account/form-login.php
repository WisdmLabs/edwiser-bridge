...
<div id="user_login">
	<?php
	wdm_show_notices();
	$action   = get_arr_value( $_GET, 'action', false );
	$username = get_arr_value( $_POST, 'username', false );
			...
			<form method="post" class="login">
				...
				if ( 'yes' === $enable_registration ) {
					...
					if ( ! empty( $_GET['redirect_to'] ) ) {
						$arg_list = '&redirect_to=' . $_GET['redirect_to'];
					}

					if ( isset( $_GET['is_enroll'] ) && 'true' === $_GET['is_enroll'] ) {
						$arg_list .= '&is_enroll=' . $_GET['is_enroll'];
					}
					?>
	...
	<?php
	if ( isset( $_GET['action'] ) && 'eb_register' === $_GET['action'] && 'yes' === $enable_registration ) {
			...
			<form method="post" class="register">
				...
				<div class="eb-user-account-terms">
					<div id = "eb-user-account-terms-content" title="<?php _e( 'Terms and Conditions', 'eb-textdomain' ); ?>">
						<?php
						echo $general_settings['eb_terms_and_cond'];
						?>
					</div>
				</div>
