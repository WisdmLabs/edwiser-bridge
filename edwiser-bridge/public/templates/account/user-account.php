	...
	if ( isset( $_GET['eb_action'] ) && 'edit-profile' === sanitize_text_field( wp_unslash( $_GET['eb_action'] ) ) ) {
		...
		<div class="eb-user-account-navigation">
			<div>
				...
				foreach ( $labels as $label ) {
					...
					if ( isset( $_GET['eb-active-link'] ) && sanitize_text_field( wp_unslash( $_GET['eb-active-link'] ) ) === $nav_href ) {
		...
		<div class="eb-user-account-content">
			...
			if ( isset( $_GET['eb-active-link'] ) ) {
				$eb_shortcode_obj->get_user_account_content( sanitize_text_field( wp_unslash( $_GET['eb-active-link'] ) ), $user_orders, $order_count, $user_avatar, $user, $user_meta, $enrolled_courses, $template_loader );
			} else {
				$template_loader->wp_get_template(
