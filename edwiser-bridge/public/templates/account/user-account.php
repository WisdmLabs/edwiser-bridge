<div class="eb-user-profile" >
	...
	if ( isset( $_GET['eb_action'] ) && 'edit-profile' === $_GET['eb_action'] ) {
		...
		<div class="eb-user-account-navigation">
			<div>
				...
				foreach ( $labels as $label ) {
					...
					if ( isset( $_GET['eb-active-link'] ) && $_GET['eb-active-link'] === $nav_href ) {
						$css_class .= ' eb-active-profile-nav';
					}
					?>
				<nav class="<?php echo $css_class; ?>">
		...
		<div class="eb-user-account-content">
			...
			if ( isset( $_GET['eb-active-link'] ) ) {
				$eb_shortcode_obj->get_user_account_content( $_GET['eb-active-link'], $user_orders, $order_count, $user_avatar, $user, $user_meta, $enrolled_courses, $template_loader );
			} else {
				$template_loader->wp_get_template(
