...
class Eb_Shortcode_My_Courses {
	...
	public static function output( $atts ) {
		...
		$my_courses = $current_class->getUserCourses( $atts['user_id'] );

		$current_class->showMyCourses( $my_courses, $atts );
		...
		if ( isset( $eb_general_setings['eb_enable_recmnd_courses'] ) && 'yes' === $eb_general_setings['eb_enable_recmnd_courses'] ) {
			if ( is_numeric( $atts['number_of_recommended_courses'] ) && $atts['number_of_recommended_courses'] > 0 ) {
				$rec_cats = $current_class->getRecommendedCategories( $my_courses );
				if ( count( $rec_cats ) || ( isset( $eb_general_setings['eb_recmnd_courses'] ) && count( $eb_general_setings['eb_recmnd_courses'] ) ) ) {
					$current_class->showRecommendedCourses( $rec_cats, $my_courses, $atts['number_of_recommended_courses'], $atts );
				}
			}
		}
	}
	...
	public function showMyCourses( $my_courses, $atts ) {
		...
		if ( ! empty( $atts['my_courses_wrapper_title'] ) ) {
			?><h2 class="eb-my-courses-h2"><?php echo $atts['my_courses_wrapper_title']; ?></h2>
			<?php
		}
		...
		if ( ! is_user_logged_in() ) {
			...
			<p>
				...
				printf(
					__( 'You are not logged in. %$1ss to login.', 'eb-textdomain' ),
			...
			<h5>
				...
				/* Translators 1: URL */
				printf(
					__( 'You are not enrolled to any course. %$1s to access the courses page.', 'eb-textdomain' ),
	...
	public function showRecommendedCourses( $rec_cats = '', $exclude_courses = '', $count = '', $atts = '', $args = '' ) {
		...
		if ( $courses->have_posts() ) {
			...
			if ( ! empty( $atts['recommended_courses_wrapper_title'] ) ) {
				?>
				<h2><?php echo $atts['recommended_courses_wrapper_title']; ?></h2>
				<?php
			}
