...
if ( ! class_exists( 'Eb_Bridge_Summary' ) ) :
	...
	class Eb_Bridge_Summary extends EBSettingsPage {
		...
		public function __construct() {
			$this->_id   = 'summary';
			$this->label = __( 'Summary', 'eb-textdomain' );
		...
		private function get_edwiser_envirment( $plugin_path ) {
			$data = array(
				array(
					'label' => __( 'Edwiser Bridge version:', 'eb-textdomain' ),
					'help'  => '',
					'value' => wdm_get_plugin_version( 'edwiser-bridge/edwiser-bridge.php' ),
				),
				// array(
				// 'label' => __( 'Edwiser Bridge Moodle version', 'eb-textdomain' ),
				// 'help'  => '',
				// 'value' => '',
				// ),
				// array(
				// 'label' => __( 'Edwiser Single Sign On Moodle version', 'eb-textdomain' ),
				// 'help'  => '',
				// 'value' => '',
				// ),
			);
			...
			foreach ( $products_data as $product ) {
				$data[] = array(
					'label' => $product['item_name'] . ' version:',
					'help'  => '',
					'value' => $this->get_plugin_version_info( $product['item_name'], $product['path'] ),
				);
			}
			$title = __( 'Edwiser Bridge Information', 'eb-textdomain' );
			require_once $plugin_path . 'partials/html-bridge-summary.php';
		}

		private function get_plugin_version_info( $name, $plugin_path = false ) {
			$version_info = '';
			if ( $plugin_path ) {
