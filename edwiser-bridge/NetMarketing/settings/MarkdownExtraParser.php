...
if (!class_exists("wisdmalbs\newtowrk\marketing\MarkdownExtraParser")) {
    include_once 'MarkdownParser.php';
    ...
    class MarkdownExtraParser extends MarkdownParser
    {
        ...
        public $contextBlockTagsRe = 'script|noscript|math|ins|del';
        ...
        public function hashHTMLBlocks($text)
        {
            ...
            # hard-coded.
            #
            # This works by calling hashHTMLBlocksInMarkdown, which then calls
            # hashHTMLBlocksInHTML when it encounter block tags. When the markdown="1"
            # attribute is found whitin a tag, hashHTMLBlocksInHTML calls back
            #  hashHTMLBlocksInMarkdown to handle the Markdown syntax within the tag.
        ...
        public function hashHTMLBlocksInMarkdown(
            $text,
            $indent = 0,
            $enclosing_tag_re = '',
            $span = false
            ...
            /*
             * Parse markdown text, calling hashHTMLBlocksInHTML for block tags.
            ...
            do {
                ...
                elseif (preg_match('{^<(?:'.$this->block_tags_re.')\b}', $tag) ||
                    ...
                    list($block_text, $text) = $this->hashHTMLBlocksInHTML($tag.$text, 'hashBlock', true);
                ...
                elseif (preg_match('{^<(?:'.$this->clean_tags_re.')\b}', $tag) ||
                    ...
                    list($block_text, $text) = $this->hashHTMLBlocksInHTML($tag.$text, 'hashClean', false);
        ...
        public function hashHTMLBlocksInHTML($text, $hash_method, $md_attr)
        {
            //
            // Parse HTML, calling hashHTMLBlocksInMarkdown for block tags.
            //
    ...
    // Returns an array of that form: ( processed text , remaining text )
                ...
                if (preg_match('{^</?(?:'.$this->auto_close_tags_re.')\b}', $tag) ||
                    ...
                    if ($md_attr &&
                        ...
                        $tag_name_re = $matches[1];
                            ...
                            list($block_text, $text) = $this->hashHTMLBlocksInMarkdown(
                                $text,
                                $indent,
                                $tag_name_re,
                                $span_mode
        ...
        public function doHeadersAttr($attr)
        {
            if (empty($attr)) {
                return '';
            }
        ...
        public function doTableLeadingPipeCallback($matches)
        {
            $head = $matches[1];
            $underline = $matches[2];
            $content = $matches[3];
        ...
        public function doTableCallback($matches)
        {
            $head = $matches[1];
            $underline = $matches[2];
            $content = $matches[3];
        ...
        public function doDefListsCallback($matches)
        {
        ...
        public function processDefListItemsCallbackDt($matches)
        {
            $terms = explode("\n", trim($matches[1]));
            $text = '';
        ...
        public function processDefListItemsCallbackDd($matches)
        {
    ...
    # ~~~
        ...
        public function doFencedCodeBlocksCallback($matches)
        {
            $codeblock = $matches[2];
        ...
        public function doFencedCodeBlocksNewlines($matches)
        {
            return str_repeat(
        ...
        public function stripFootnotesCallback($matches)
        {
        ...
        public function appendFootnotesCallback($matches)
        {
        ...
        public function stripAbbreviationsCallback($matches)
        {
            $abbr_word = $matches[1];
            $abbr_desc = $matches[2];
            if ($this->abbr_word_re) {
        ...
        public function doAbbreviationsCallback($matches)
        {
            $abbr = $matches[0];
