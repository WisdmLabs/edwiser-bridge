...
if (!class_exists("wisdmalbs\newtowrk\marketing\MarkdownParser")) {
    class MarkdownParser
    {
        ...
        public $nestUrlParenthsDepth = 4;
        public $nestUrlParenthRe;
        ...
        public function __construct()
        {
            ...
            $this->nested_brackets_re = str_repeat('(?>[^\[\]]+|\[', $this->nestedBracketsDepth).
                    str_repeat('\])*', $this->nestedBracketsDepth);

            $this->nestUrlParenthRe = str_repeat('(?>[^()\s]+|\(', $this->nestUrlParenthsDepth).
                    str_repeat('(?>\)))*', $this->nestUrlParenthsDepth);
...
//            $in_anchor = false;
        ...
        public function stripLinkDefinitionsCallback($matches)
        {
        ...
        public function hashHTMLBlocksCallback($matches)
        {
            $text = $matches[1];
    ...
    # The $boundary argument specify what character should be used to surround
        ...
        public function doHardBreaksCallback($matches)
        {
            unset($matches);
        ...
        public function doAnchorsReferenceCallback($matches)
        {
        ...
        public function doAnchorsInlineCallback($matches)
        {
            $whole_match = $matches[1];
			...
			(				# wrap whole match in $1
			  ...
			  \(			# literal paren
				...
				(?:
					<(\S*)>	# src url = $3
				|
					('.$this->nestUrlParenthRe.')	# src url = $4
				)
				[ \n]*
				(			# $5
				  ([\'"])	# quote char = $6
        ...
        public function doImagesReferenceCallback($matches)
        {
            $whole_match = $matches[1];
            $alt_text = $matches[2];
        ...
        public function doImagesInlineCallback($matches)
        {
            $whole_match = $matches[1];
            $alt_text = $matches[2];
            $url = $matches[3] == '' ? $matches[4] : $matches[3];
        ...
        public function doHeadersCallbackSetext($matches)
        {
        ...
        public function doHeadersCallbackAtx($matches)
        {
        ...
        public function doListsCallback($matches)
        {
            # Re-usable patterns to match list item bullets and number markers:
            $marker_ul_re = '[*+-]';
            $marker_ol_re = '\d+[.]';
        ...
        public function processListItemsCallback($matches)
        {
            $item = $matches[4];
        ...
        public function doCodeBlocksCallback($matches)
        {
            $codeblock = $matches[1];
        ...
        public function prepareItalicsAndBold()
        {
            ...
            # context.
            #
            foreach ($this->em_relist as $key => $em_re) {
                foreach ($this->strong_relist as $strong => $strong_re) {
                    # Construct list of allowed token expressions.
                    $token_relist = array();
                    if (isset($this->em_strong_relist["$key$strong"])) {
                        $token_relist[] = $this->em_strong_relist["$key$strong"];
                    }
                    $token_relist[] = $em_re;
                    ...
                    $this->emStrPrepRelist["$key$strong"] = $token_re;
                }
            }
        }
        ...
        public function doItalicsAndBold($text)
        {
            ...
            while (1) {
                ...
                # in current context.
                #
                $token_re = $this->emStrPrepRelist["$emphasis$strong"];
        ...
        public function doBlockQuotesCallback($matches)
        {
            $backQuots = $matches[1];
            # trim one level of quoting - trim whitespace-only lines
            $backQuots = preg_replace('/^[ ]*>[ ]?|^[ ]+$/m', '', $backQuots);
            $backQuots = $this->runBlockGamut($backQuots);        # recurse

            $backQuots = preg_replace('/^/m', '  ', $backQuots);
            # These leading spaces cause problem with <pre> content,
            # so we need to fix that:
            $backQuots = preg_replace_callback(
                '{(\s*<pre>.+?</pre>)}sx',
        ...
        public function doBlockQuotesCallback2($matches)
        {
            $pre = $matches[1];
        ...
        public function doAutoLinksUrlCallback($matches)
        {
        ...
        public function doAutoLinksEmailCallback($matches)
        {
            $address = $matches[1];
    ...
    #	Based by a filter by Matthew Wickline, posted to BBEdit-Talk.
        ...
        public function detabCallback($matches)
        {
            $line = $matches[0];
        ...
        public function unhashCallback($matches)
        {
