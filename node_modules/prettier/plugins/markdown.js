...
`,Dr="	",Ye=" ",BD="~",Ju="`",qD=3,TD=4;function SD(e,r,n){var t=this,a=t.options.gfm,i=r.length+1,u=0,o="",s,l,c,f,D,h,p,d,m,g,x,F,B;if(a){for(;u<i&&(c=r.charAt(u),!(c!==Ye&&c!==Dr));)o+=c,u++;if(F=u,c=r.charAt(u),!(c!==BD&&c!==Ju)){for(u++,l=c,s=1,o+=c;u<i&&(c=r.charAt(u),c===l);)o+=c,s++,u++;if(!(s<qD)){for(;u<i&&(c=r.charAt(u),!(c!==Ye&&c!==Dr));)o+=c,u++;for(f="",p="";u<i&&(c=r.charAt(u),!(c===jr||l===Ju&&c===l));)c===Ye||c===Dr?p+=c:(f+=p+c,p=""),u++;if(c=r.charAt(u),!(c&&c!==jr)){if(n)return!0;B=e.now(),B.column+=o.length,B.offset+=o.length,o+=f,f=t.decode.raw(t.unescape(f),B),p&&(o+=p),p="",g="",x="",d="",m="";for(var b=!0;u<i;){if(c=r.charAt(u),d+=g,m+=x,g="",x="",c!==jr){d+=c,x+=c,u++;continue}for(b?(o+=c,b=!1):(g+=c,x+=c),p="",u++;u<i&&(c=r.charAt(u),c===Ye);)p+=c,u++;if(g+=p,x+=p.slice(F),!(p.length>=TD)){for(p="";u<i&&(c=r.charAt(u),c===l);)p+=c,u++;if(g+=p,x+=p,!(p.length<s)){for(p="";u<i&&(c=r.charAt(u),!(c!==Ye&&c!==Dr));)g+=c,x+=c,u++;if(!c||c===jr)break}}}for(o+=d+g,u=-1,i=f.length;++u<i;)if(c=f.charAt(u),c===Ye||c===Dr)D||(D=f.slice(0,u));else if(D){h=f.slice(u);break}return e(o)({type:"code",lang:D||f||null,meta:h||null,value:m})}}}}}});var Oe=C((Ve,Zu)=>{Ve=Zu.exports=LD;function LD(e){return e.trim?e.trim():Ve.right(Ve.left(e))}Ve.left=function(e){return e.trimLeft?e.trimLeft():e.replace(/^\s\s*/,"")};Ve.right=function(e){if(e.trimRight)return e.trimRight();for(var r=/\s/,n=e.length;r.test(e.charAt(--n)););return e.slice(0,n+1)}});var $r=C((DF,ea)=>{"use strict";ea.exports=PD;function PD(e,r,n,t){for(var a=e.length,i=-1,u,o;++i<a;)if(u=e[i],o=u[1]||{},!(o.pedantic!==void 0&&o.pedantic!==n.options.pedantic)&&!(o.commonmark!==void 0&&o.commonmark!==n.options.commonmark)&&r[u[0]].apply(n,t))return!0;return!1}});var ia=C((pF,na)=>{"use strict";var _D=Oe(),OD=$r();na.exports=ID;var Rt=`
`,ra="	",zt=" ",ta=">";function ID(e,r,n){for(var t=this,a=t.offset,i=t.blockTokenizers,u=t.interruptBlockquote,o=e.now(),s=o.line,l=r.length,c=[],f=[],D=[],h,p=0,d,m,g,x,F,B,b,v;p<l&&(d=r.charAt(p),!(d!==zt&&d!==ra));)p++;if(r.charAt(p)===ta){if(n)return!0;for(p=0;p<l;){for(g=r.indexOf(Rt,p),B=p,b=!1,g===-1&&(g=l);p<l&&(d=r.charAt(p),!(d!==zt&&d!==ra));)p++;if(r.charAt(p)===ta?(p++,b=!0,r.charAt(p)===zt&&p++):p=B,x=r.slice(p,g),!b&&!_D(x)){p=B;break}if(!b&&(m=r.slice(p),OD(u,i,t,[e,m,!0])))break;F=B===p?x:r.slice(B,g),D.push(p-B),c.push(F),f.push(x),p=g+1}for(p=-1,l=D.length,h=e(c.join(Rt));++p<l;)a[s]=(a[s]||0)+D[p],s++;return v=t.enterBlock(),f=t.tokenizeBlock(f.join(Rt),o),v(),h({type:"blockquote",children:f})}}});var oa=C((dF,aa)=>{"use strict";aa.exports=RD;var ua=`
`,pr="	",dr=" ",hr="#",ND=6;function RD(e,r,n){for(var t=this,a=t.options.pedantic,i=r.length+1,u=-1,o=e.now(),s="",l="",c,f,D;++u<i;){if(c=r.charAt(u),c!==dr&&c!==pr){u--;break}s+=c}for(D=0;++u<=i;){if(c=r.charAt(u),c!==hr){u--;break}s+=c,D++}if(!(D>ND)&&!(!D||!a&&r.charAt(u+1)===hr)){for(i=r.length+1,f="";++u<i;){if(c=r.charAt(u),c!==dr&&c!==pr){u--;break}f+=c}if(!(!a&&f.length===0&&c&&c!==ua)){if(n)return!0;for(s+=f,f="",l="";++u<i&&(c=r.charAt(u),!(!c||c===ua));){if(c!==dr&&c!==pr&&c!==hr){l+=f+c,f="";continue}for(;c===dr||c===pr;)f+=c,c=r.charAt(++u);if(!a&&l&&!f&&c===hr){l+=c;continue}for(;c===hr;)f+=c,c=r.charAt(++u);for(;c===dr||c===pr;)f+=c,c=r.charAt(++u);u--}return o.column+=s.length,o.offset+=s.length,s+=l+f,e(s)({type:"heading",depth:D,children:t.tokenizeInline(l,o)})}}}});var la=C((hF,ca)=>{"use strict";ca.exports=$D;var zD="	",MD=`
