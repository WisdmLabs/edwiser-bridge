 ...
 * @subpackage Edwiser Bridge/includes/payments
...
class EB_Payment_Manager {
	...
	function paypal_rewrite_rules( $wp_rewrite ) {
		$wp_rewrite->rules = array_merge( array( 'eb/paypal-notify' => 'index.php?eb=paypal-notify' ), $wp_rewrite->rules );
	}
	...
	public function parse_ipn_request( $wp ) {
		if ( array_key_exists( 'eb', $wp->query_vars )
			&& $wp->query_vars['eb'] == 'paypal-notify' ) {
	...
	public static function access_course_button( $course_id ) {
		...
		$moodle_course_id   = EB()->course_manager()->get_moodle_course_id( $course_id );
		$access_course_url  = '';
		$access_button      = '';
		...
		if ( empty( $access_course_url ) )
			...
			$access_button = '<div class="eb_join_button"><a class="wdm-btn" href="'.$access_course_url.'" id="wdm-btn">'.__( "Access Course", "eb-textdomain" ).'</a></div>';
		}
	...
	public static function take_course_button( $course_id ) {
		...
		$course_price_type  = isset( $course_meta['course_price_type'] )?$course_meta['course_price_type']:'free';
		$course_price       = isset( $course_meta['course_price'] )?$course_meta['course_price']:0;
		$closed_button_url  = isset( $course_meta['course_closed_url'] )?$course_meta['course_closed_url']:'#';
		...
		$paypal_sandbox   = ( $paypal_sandbox == 'no' )? 0:1;
		...
		if ( !is_user_logged_in() && $course_price_type == "closed" ) {
			if ( empty( $closed_button_url ) )
				...
				$closed_button = '<div class="eb_join_button"><a class="wdm-btn" href="'.$closed_button_url.'" id="wdm-btn">'.__( "Take this Course", "eb-textdomain" ).'</a></div>';
			}
			...
			$login_url =  wdm_user_account_url( '?redirect_to='.get_permalink( $course_id ) );
			$login_button = '<div class="eb_join_button"><a class="wdm-btn" href="'.$login_url.'" id="wdm-btn">'.__( "Take this Course", "eb-textdomain" ).'</a></div>';
		 ...
		 */
		if ( EB()->enrollment_manager()->user_has_course_access( $user_id, $course_id ) ) {
			return '';
		}

		if ( !empty( $course_price_type ) && $course_price_type == "closed" ) { //closed course button
			if ( empty( $closed_button_url ) )
				$closed_button = '';
			else {
				if ( !strpos( $closed_button_url, "://" ) )
			...
			return $closed_button;
		} elseif ( ( !empty( $course_price_type ) && $course_price_type == "free" ) || ( $course_price_type == "paid" && empty( $course_price ) ) ) { //free course button
			$free_button = '<div class="eb_join_button"><form method="post">
                            <input type="hidden" value="'.$course->ID.'" name="course_id">
                            <input type="submit" value="'.__( "Take this Course", "eb-textdomain" ).'" name="course_join" class="wdm-btn" id="wdm-btn">
                        </form></div>';

			return apply_filters( 'eb_course_free_button', $free_button, $course->ID );

		} elseif ( !empty( $course_price ) && !empty( $course_price_type ) && $course_price_type == "paid" ) { //paid course button
			require_once 'enhanced-paypal-shortcodes.php';

			$paypal_button = '';
			if ( !empty( $paypal_email ) ) {
				$paypal_button = wptexturize( do_shortcode( "[paypal type='paynow' amount='{$course_price}' sandbox='{$paypal_sandbox}' email='{$paypal_email}' itemno='{$course->ID}' name='{$course->post_title}' noshipping='1' nonote='1' qty='1' currencycode='{$paypal_currency}' rm='2' notifyurl='{$paypal_notifyurl}' returnurl='{$paypal_returnurl}' scriptcode='scriptcode' imagewidth='100px' pagestyle='paypal' lc='{$paypal_country}' cbt='" . __( 'Complete Your Purchase', 'eb-textdomain' ) . "' custom='".$user_id."']" ) );
				...
				$not_purchasable = apply_filters( 'eb_course_not_purchasable_notice', __( 'Course Not Available', 'eb-textdomain' ) );
	...
	//   return "";
	//  else
	//   return $this->take_course_button( $course_id );
	// }
	//
}
